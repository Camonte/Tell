		...
		private void DoRaycast2D(Camera camera, Vector2 screenPosition, ref Component bestResult, ref float bestDistance, ref Vector3 bestPosition)
		{
			...
			if (count > 0)
			{
				for (int i = 0; i < count; i++)
				{
					...
					var taggedHit = !hit.transform.CompareTag("Untagged"); // PETER: ugly way of giving priority to tagged elements (grapheme, phoneme, generator...)

					if (distance <= bestDistance || (distance == bestDistance && taggedHit))
					{
						bestDistance = distance;
						bestResult = hit.transform;
